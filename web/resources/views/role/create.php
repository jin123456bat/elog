...
<body>
	...
	<div class="container">
		...
		<div class="main">
			...
			<div class="body">
				<form class="form" id="baseForm" action="<?=url('role/update')?>">
					<div class="white-block">
						<div class="form-title">基础信息</div>
						<div class="form-group col-md-10">
							<label class="label col-md-1">角色名称</label>
							<input type="text" class="input_text col-md-2" name="name" value="" placeholder="角色名称">
						</div>
						<div class="form-group col-md-10">
							<label class="label col-md-1">备注</label>
							<input type="text" class="input_text col-md-2" name="note" value="" placeholder="备注">
						</div>
					</div>
					<div class="line"></div>
					<div class="white-block">
...
<script>
	...
	$('.button-submit').on('click',function(){
		...
		$this.loading('start');
		$.post('<?=url('role/create')?>',{name:name,note:note,privileges:privileges},function(response){
			$this.loading('stop');
			if(response.code==1)
			{
				window.location = '<?=url('role/index')?>';
				return false;
			}
			spop({
