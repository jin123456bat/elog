		...
		<div class="main">
			...
			<div class="body">
				<div class="white-block">
					...
					<div class="col-md-10">
						<form id="articleForm" class="form" method="post" action="<?=url('cms/article_update')?>">
							<div class="panel col-md-7 center-block">
								...
								<div class="panel-body">
									...
									<div class="form-group col-md-10">
										...
										<input type="text" class="input_text col-md-7" name="title" value="<?=$article['title']?>" placeholder="">
									</div>
									<?php if (!empty($article->category_id_list())){?>
									<?php foreach ($article->category_id_list() as $index => $category_id){?>
									<div class="form-group col-md-10">
									...
									<div class="form-group col-md-10">
										...
										<textarea class="textarea col-md-7" rows="5" name="short_desc"><?=$article['short_desc']?></textarea>
									</div>
									<div class="form-group col-md-10">
										<label class="label col-md-2">标签</label>
										<input type="text" class="input_text col-md-7" name="tags" value="<?=$article['tags']?>" placeholder="">
										<span class="col-offset-2 text-helper">多个标签使用逗号分隔</span>
									</div>
									<div class="form-group col-md-10">
										<label class="label col-md-2">排序</label>
										<input type="text" class="input_text col-md-7" name="sort" value="<?=$article['sort']?>" placeholder="">
										<span class="col-offset-2 text-helper">从小到大</span>
									</div>
									<div class="form-group col-md-10">
										<label class="label col-md-2">置顶</label>
										<div class="col-md-7 checkbox">
											<input type="checkbox" name="top" id="top" <?=$article['top']==1?'checked="checked"':''?>>
									...
									<div class="form-group col-md-10">
										...
										<div class="col-md-7 checkbox">
											<input type="checkbox" name="footer1" id="footer1" <?=$article['footer1']==1?'checked="checked"':''?>>
									...
									<div class="form-group col-md-10">
										...
										<div class="col-md-7 checkbox">
											<input type="checkbox" name="footer2" id="footer2" <?=$article['footer2']==1?'checked="checked"':''?>>
							...
							<div class="panel col-md-7 center-block">
								...
								<div class="panel-body" id="content"><?=$article['content']?></div>
							</div>
...
$(function(){
	...
	$('#image_area').upload({
		<?php if (!empty($article['logo_id'])){?>
		init:[<?=json_encode(['id'=>$article['logo_id'],'url'=>helper::getImageUrl($article['logo_path'])])?>],
		<?php }?>
		width:100,
		height:100,
