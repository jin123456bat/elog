		...
		<div class="main">
			...
			<div class="body">
				<div class="white-block">
					...
					<div class="col-md-10">
						<form id="categoryForm" class="form" method="post" action="<?=url('cms/category_update')?>">
							<div class="panel col-md-7 center-block">
								...
								<div class="panel-body">
									...
									<div class="form-group col-md-10">
										...
										<input type="text" class="input_text col-md-7" name="name" value="<?=$category['name']?>" placeholder="">
									</div>
									<div class="form-group col-md-10">
										<label class="label col-md-2">上级分类</label>
										<select class="select col-md-7" name="pid">
											<option value="">顶级分类</option>
											<?php foreach ($category_list as $m){?>
											<option value="<?=$m['id']?>" <?=$category['pid'] == $m['id']?'selected="selected"':''?>><?=$m['name']?></option>
											<?php }?>
										</select>
									</div>
									<div class="form-group col-md-10">
										<label class="label col-md-2">排序</label>
										<input type="text" name="sort" class="input_text col-md-7" value="<?=$category['sort']?>">
										<span class="text-helper col-offset-2">从小到大</span>
									</div>
									<div class="form-group col-md-10">
										<label class="label col-md-2">置顶</label>
										<div class="checkbox col-md-7">
											<input type="checkbox" name="top" id="top" <?=$category['top']?'checked="checked"':''?>>
...
$(function(){
	...
	$('#image_area').upload({
		<?php if (!empty($category['logo_id'])){?>
		init:[<?=json_encode(['id'=>$category['logo_id'],'url'=>helper::getImageUrl($category['logo_path'])])?>],
		<?php }?>
		width:100,
		height:100,
	...
	$('form').on('submit',function(){
		var data = {
			...
			pid:$.trim($(this).find('select[name=pid]').val()),
		...
		$.post($(this).attr('action'),data,function(response){
			...
			if(response.code==1)
			{
				window.location = '<?=url('cms/category_index')?>';
			}
			else
			{
				spop({
