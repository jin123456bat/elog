		...
		<div class="main">
			...
			<div class="body">
				<div class="white-block">
					...
					<div class="line"></div>
					<form class="form" id="form" method="post" action="<?=url('activity/update')?>">
						...
						<div class="panel center-block">
							...
							<div class="panel-body">
								<div class="form-group col-md-10">
									...
									<div class="col-md-7">
										<img class="upload_image" src="<?=empty($activity['logo_path'])?assets::common('plus.png', 'image'):$activity['logo_path']?>" style="border: 1px solid #ccc; cursor: pointer; max-width: 100%;" src="<?=assets::common('plus.png', 'image')?>">
										<input type="hidden" name="upload_logo_id" value="<?=$activity['logo_id']?>">
								...
								<div class="form-group col-md-10">
									...
									<input type="text" class="input_text col-md-7" name="desc_url" value="<?=$activity['desc_url']?>">
									<span class="text-helper col-offset-2">活动详情页地址，点击LOGO图或者跳转链接的时候调转到该地址</span>
								</div>
								<div class="form-group col-md-10">
									<label class="label col-md-2">活动名称</label>
									<input type="text" class="input_text col-md-7" name="name" value="<?=$activity['name']?>">
								...
								<div class="form-group col-md-10">
									...
									<select class="select col-md-7" name="event">
										<option value="register_member" <?=$activity['event']=='register_member'?'selected="selected"':''?>>注册会员</option>
										<option value="subscribe_wechat" <?=$activity['event']=='subscribe_wechat'?'selected="selected"':''?>>关注微信公众号</option>
									</select>
								</div>
								<div class="form-group col-md-10">
									<label class="label col-md-2">奖励类型</label>
									<select class="select col-md-7" name="type">
										<option value="money" <?=$activity['type']=='money'?'selected="selected"':''?>>余额</option>
										<option value="score" <?=$activity['type']=='score'?'selected="selected"':''?>>积分</option>
										<option value="coupon" <?=$activity['type']=='coupon'?'selected="selected"':''?>>优惠券</option>
									</select>
								</div>
								<div class="form-group col-md-10 display-none money-content">
									<label class="label col-md-2">余额</label>
									<input type="text" class="input_text col-md-7" name="money_content" value="<?=$activity['type']=='money'?$activity['content']/100:''?>">
								...
								<div class="form-group col-md-10 display-none score-content">
									...
									<input type="text" class="input_text col-md-7" name="score_content" value="<?=$activity['type']=='score'?$activity['content']:''?>">
								...
								<div class="form-group col-md-10 display-none coupon-content">
									...
									<select class="select col-md-7" name="coupon_content">
										<?php foreach ($coupon as $c){?>
										<option value="<?=$c['id']?>" <?=$activity['type']=='coupon'&&$activity['content']==$c['id']?'selected="selected"':''?>><?=$c['name']?></option>
										<?php }?>
									</select>
								...
								<div class="form-group col-md-10">
									...
									<input type="text" class="input_text col-md-7" name="active_num_per" placeholder="0不限制次数" value="<?=$activity['active_num_per']?>">
								...
								<div class="form-group col-md-10">
									...
									<div style="display: inline; position: relative;">
										<input type="text" class="input_text col-md-3 datetimepicker" name="starttime" value="<?=$activity['starttime']?>" placeholder="开始时间，不填写则不限制" autocomplete="off">
									</div>
									<div class="col-md-1" style="text-align: center; width: 7%;">~</div>
									<div style="display: inline; position: relative;">
										<input type="text" class="input_text col-md-3 datetimepicker" name="endtime" value="<?=$activity['endtime']?>" placeholder="结束时间，不填写则不限制" autocomplete="off">
									</div>
								...
								<div class="form-group col-md-10">
									...
									<div class="col-md-7 checkbox">
										<input type="checkbox" id="status" name="status" value="1" <?=$activity['status']==1?('checked="checked"'):''?>>
